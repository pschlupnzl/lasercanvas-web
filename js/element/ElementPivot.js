/**
* LaserCanvas - ring-closing element. This is used when dragging across the
* closing gap in a ring cavity.
*/
(function (LaserCanvas) {
    LaserCanvas.Element.Pivot = function (source) {
        this.type = "Pivot"; // {string} Primitive element type.
        this.name = "Z"; // {string} Name of this element (updated by System).
        this.variablesGetter = source.variablesGetter; // {function} Used to retrieve variable values.
    ...
    LaserCanvas.Element.Pivot.prototype = {
        ...
        /** Set the source element, i.e. the first mirror in the system. */
        setRing: function (element) {
            console.log(`setRing`, element)
            this.source = element;
        },

        /** Returns FALSE indicating that the element cannot be selected. */
        atLocation: function (pt_notused, tol_notused) {
            console.log(`«« atLocation - nope`)
            return false;
        },
        ...
        */
        location: function (ax) {
            // console.log(`«» location -> ${JSON.stringify(ax)}`)
            if (ax) {
                console.warn(`«» ∆∆ location set not implemented`)
            }
            // if (ax) {
            //     this.loc.x = ax.x;
            //     this.loc.y = ax.y;
        ...
        */
        userProperties: function () {
            ...
            //     standard: LaserCanvas.Element.Ring.standard // {Array<number>} Standard values.
            // }];
        ...
        */
        canSetProperty: function (propertyName) {
            if (!["outgoingAngle"].includes(propertyName)) {
                console.warn(`««« ∆∆ canSetProperty ${propertyName} not implemented`)
            }
            console.log(`««« canSetProperty("${propertyName}")`)
            return {
        ...
        set: function (propertyName, newValue, arg) {
            if (!["outgoingAngle", "deflectionAngle", "angleOfIncidence"].includes(propertyName)) {
                console.warn(`» ∆∆ set ${propertyName} ${newValue} not implemented`)
            }
            switch (propertyName) {
                // case "distanceToNext":
        ...
        /**
        * Return the ABCD matrix for this element. Since this is a virtual
        * to close the ring, it does not contribute to the propagation matrix
        ...
        */
        elementAbcd: function (dir_notused, plane_notused) {
            return LaserCanvas.Math.Matrix2x2.eye();
        ...
        // /**
        //  * Updates the first element in the cavity to geometrically close the
        //  * ring.
        //  * @param {Element.Mirror} element The linking element, i.e. the first mirror.
        //  */
        // closeRing: function (element) {
        //     var loc = element.location();
        //     console.log(`elementLoc=`, loc);
        //     console.log('my loc=', this.loc);
        //     //  /|\  TODO
        //     // /_._\ Figure out which direction!
        //     // Location: Transfer up or down, depending on the action.
        //     if (true) {
        //         // Move my updated location to the starting element.
        //         loc.x = this.loc.x;
        //         loc.y = this.loc.y;
        //     }
        //     // Deflection angle.
        //     loc.q = -element.get('deflectionAngle');
        //     element.location(loc);
        //     console.log(`closeRing`, element)
        // },
        ...
        */
        wireframe: function (render, layer) {
            var //k, dx,
                renderLayer = LaserCanvas.Enum.renderLayer,    // {Enum} Layer to draw.
                d = "M -16 0 L 16 0 M 0 -8 L 0 8"; // {string} Path drawing instructions.
            ...
            switch (layer) {
                case renderLayer.optics:
                    ...
                    render
                        ...
                        .setStroke("#000", 2)
                        .beginPath()
                        .drawPath(d, this.loc.x, this.loc.y, qc)
                        .stroke()
                        .restore();
                break;
